...
from datasets.GEOM import GEOM
...
from engine_geom import train_one_epoch, evaluate, compute_stats
...
def get_args_parser():
    ...
    parser.add_argument("--data-path", type=str, default='data/GEOM')
...
def main(args):
    ...
    ''' Dataset '''
    train_dataset = GEOM(args.data_path, 'train', feature_type=args.feature_type)
    val_dataset   = GEOM(args.data_path, 'valid', feature_type=args.feature_type)
    np.random.seed(0)
    train_random_list = np.random.randint(0, len(train_dataset), 200000).tolist()
    val_random_list = np.random.randint(0, len(val_dataset), 10000).tolist()
    train_dataset = train_dataset[train_random_list]
    val_dataset = val_dataset[val_random_list]
    _log.info('Sampling 200,000 molecules for training and 10,000 for val')
    task_mean, task_std = -80.5800, 15.8439
    # max label: -7.4734 min label: -281.2706
    ...
    for epoch in range(args.epochs):
        ...
        train_err = train_one_epoch(model=model, criterion=criterion, norm_factor=norm_factor,
            ...
            print_freq=args.print_freq, logger=_log, ssp=args.ssp)
        ...
        info_str = 'Best -- epoch={}, train MAE: {:.5f}, val MAE: {:.5f}\n'.format(
            best_epoch, best_train_err, best_val_err)
        _log.info(info_str)
